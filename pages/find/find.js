...
onShow(){
 ...
 if(today==new Date().getDate()){
  ...
  onReachBottom() {
    ...
    this.getPetPointList(++this.data.pageno)
    //}
  },
  // 获取战队
  getHotTroopList(){
    let that = this;
    wx.request({
      url: 'https://wechatapi.vipcsg.com/index/team/index',
      data: {
        page: 1,
        t:2
      },
  ...
  // 获取明星萌宠
  getPetPointList(pageno){
    let that = this;
    wx.request({
      url: 'https://wechatapi.vipcsg.com/index/eventslike/all_pet',
      data: {
        page: pageno
      },
      success(res) {
        that.setData({
          // goods_index: that.data.goods_index.concat(res.data.data)
          petPointList: that.data.petPointList.concat(res.data.data)
        })
        console.log(that.data.petPointList)
      },
    })
  },
  //请求商品列表
  getGoodsIndex(pageno) {
    let that = this;
    wx.request({
      url: 'https://wechatapi.vipcsg.com/index/member/pet_details?user_id=19&pet_id=61',
      data: {
        page: pageno
      },
      success(res) {
        that.setData({
          // goods_index: that.data.goods_index.concat(res.data.data)
          goods_index:res.data.data.pet_img
        })
        console.log(that.data.goods_index)
      },
    })
